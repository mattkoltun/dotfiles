...
FMT_VCS_STATUS="%F{magenta}on %B%{$oxide_turquoise%} %b%u%c%{$oxide_reset_color%}"
...
# Oxide prompt style.
PROMPT=$'\n%B%{$oxide_orange%}%n%{$oxide_reset_color%}%b %F{magenta}at %B%{$oxide_turquoise%}%m%{$oxide_reset_color%}%b %F{magenta}in %{$oxide_limegreen%}%~%{$oxide_reset_color%} ${vcs_info_msg_0_}\n%(?.%{%F{white}%}.%{$oxide_red%})%b%(!.#.❯)%{$oxide_reset_color%} '
